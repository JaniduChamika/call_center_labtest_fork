...
export async function sendAppointmentEmail(appointment_id: string) {
  try {
    ...
    // 1️⃣ Fetch appointment details + Doctor's Fee
    const appointment = await prisma.appointments.findUnique({
      where: { public_id: appointment_id },
      include: {
        patients: true,
        doctors: true, // This brings in the consultant_fee
        hospitals: true,
      },
    });
    ...
    // 2️⃣ Calculate Fee (formatting it to LKR)
    const totalFee = appointment.doctors.consultant_fee
      ? `LKR ${Number(appointment.doctors.consultant_fee).toLocaleString()}`
      : "N/A";

    // 3️⃣ Update HTML template with the Total Payment row
    const emailHTML = `
  <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 10px; background-color: #f9f9f9;">
    <h2 style="text-align: center; color: #4CAF50;">Appointment Confirmation</h2>
    <p>Dear <strong>${appointment.patients.name}</strong>,</p>
    ...
    <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
      ...
      <tr>
        <td style="padding: 10px; font-weight: bold; color: #4CAF50;">Total Payment</td>
        <td style="padding: 10px; font-weight: bold; color: #4CAF50;">${totalFee}</td>
      </tr>
      <tr style="background-color: #f1f1f1;">
        <td style="padding: 10px; font-weight: bold;">Action</td>
        <td style="padding: 10px;">
          <a href="${appointment.payment_link}" style="background-color: #4CAF50; color: white; padding: 8px 12px; text-decoration: none; border-radius: 5px; display: inline-block;">Pay Now</a>
        </td>
      </tr>
    </table>
...
`;
    ...
    await transporter.sendMail({
      from: process.env.EMAIL_USER!,
      to: patientEmail,
      subject: `Appointment Confirmed - Dr. ${appointment.doctors.name}`,
      html: emailHTML,
    });
